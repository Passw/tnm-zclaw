...
The **888 KB** target is all-in firmware size, not just app code. It includes zclaw logic plus ESP-IDF/FreeRTOS runtime, Wi-Fi/networking, TLS/crypto, and cert bundle overhead.
...
`zclaw.bin` from the same build is `865888` bytes (~845.6 KiB), which stays under the `888 KB` cap.
